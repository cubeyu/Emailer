...
public class Metrics {
	 ...
	 */
	final OnlinePlayers onlinePlayers = new OnlinePlayers();
	...
	private JsonObject getServerData() {
		...
		data.addProperty("playerAmount", onlinePlayers.getPlayers());
	...
	public class OnlinePlayers implements PluginMessageListener {
		...
		public OnlinePlayers() {
	        ...
	        if (!messenger.isIncomingChannelRegistered(CommonUtils.getPlugin(), "BungeeCord")) {
	        	messenger.registerIncomingPluginChannel(CommonUtils.getPlugin(), "BungeeCord", this);
	        }
	        
	        boolean isBungee = CommonUtils.isBungee();
	        ...
	        new BukkitRunnable() {
				...
				public void run() {
					...
					if (isBungee) {
					    ...
					    output.writeUTF("ALL");
					    player.sendPluginMessage(CommonUtils.getPlugin(), "BungeeCord", output.toByteArray());
					} else {
						playerAmount = players.size();
					}
				}
			}.runTaskTimer(CommonUtils.getPlugin(), 20, 20 * 60 * 10); //Run every 20 minutes
		}
